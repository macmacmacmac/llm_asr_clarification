...
transcript_files = ['whisper_tiny_diarized_transcript']
...
for t in transcript_files:
    for q in question_files:
        command = f"sbatch -t 60 cpu_job.sh --scripts quiz_pipeline.question_answerer quiz_pipeline.question_scorer --transcript_file {t} --do_all_meetings --question_file {q} --model_to_use gpt-5.4-mini"
        os.system(command)
