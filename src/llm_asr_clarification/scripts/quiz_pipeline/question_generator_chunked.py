...
from llm_asr_clarification.models.prompts import QUIZ_QUESTION_GENERATOR_CHUNKED_PROMPT
...
def run(args_list=None):
    ...
    for meeting_path in tqdm(meeting_paths):
        ...
        for sentence_chunk in chunk_sentences(sentences, chunk_size=args.chunk_size):
            ...
            prompt = QUIZ_QUESTION_GENERATOR_CHUNKED_PROMPT.format(
            ...
            if "quiz_question" in result:
                questions.append(
                    f"Question: {result.get("quiz_question")}\nAnswer: {result.get("correct_answer")}"
                )
